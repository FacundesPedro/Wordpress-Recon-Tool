...
class FfufWpStep(BaseToolStep):
    ...
    def build_command(self) -> list[str]:
        """Build FFUF command for WordPress path discovery."""
        cmd = [
            self._tool_binary,
            "-u",
        ...
        for line in lines:
            ...
            findings.append(
                Finding(
                    ...
                    step=self.name,
                    severity="info",
                    title=f"WordPress Path Found: {word}",
                    description="WordPress-specific path discovered via FFUF fuzzing",
                    evidence=f"URL: {url}\nStatus: {status}\nSize: {length}",
    ...
    async def run(self) -> list[Finding]:
        """Execute FFUF WordPress path discovery."""
        ...
        self.logger.info(f"Running FFUF WordPress path discovery on {self.target.url}")
        ...
        try:
            ...
            if result.stdout:
                ...
                self.logger.info(f"FFUF completed: {len(self.findings)} WordPress paths found")
            else:
