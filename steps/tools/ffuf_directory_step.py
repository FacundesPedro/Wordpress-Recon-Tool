...
class FfufDirectoryStep(BaseToolStep):
    """FFUF directory discovery step.
    ...
    def __init__(
        ...
        default_wordlist = get_wordlist_path("ffuf/directories.txt") or (
            DEFAULT_WORDLISTS_DIR / "directories.txt"
        )
        self.wordlist = (
    ...
    def build_command(self) -> list[str]:
        """Build FFUF command for directory discovery."""
        cmd = [
            self._tool_binary,
            "-u",
        ...
        for line in lines:
            ...
            findings.append(
                Finding(
                    ...
                    description="Hidden directory discovered via FFUF fuzzing",
                    evidence=f"URL: {url}\nStatus: {status}\nSize: {length}",
                    recommendation="Review directory contents and access controls",
    ...
    async def run(self) -> list[Finding]:
        """Execute FFUF directory discovery."""
        ...
        self.logger.info(f"Running FFUF directory discovery on {self.target.url}")
        ...
        try:
            ...
            if result.stdout:
                ...
                self.logger.info(f"FFUF completed: {len(self.findings)} directories found")
            else:
